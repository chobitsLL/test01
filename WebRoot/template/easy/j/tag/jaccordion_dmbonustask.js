//新版EasyUI DM单转发佣金任务手风琴20170905
function jaccordion(parent,width,isEidt){
	if(width==undefined){
		width = 400;//默认宽度
	}
	var accordion = {
		p_group:$('<div class="panel-group" style="margin:0 auto;border-width:0px;" id="accordion"></div>'),
		p_add:$('<span style="margin-top:5px;">&nbsp;增加DM单转发佣金任务&nbsp;</span>'),
		p_data:new Array,
		p_index:0,
		//初始化
		init:function(){
		...
		isEmpty:function(){ // 返回true时，至少有一个奖项卡的必填项未填
			...
			for(var i=0;i<data.insertd.length;i++) {
				...
				if(prize.FDMInfoID == ""){
					...
					return true;
				}else if(prize.FForwardRead == "" && prize.FForwardScore == ""){
...
function addPanel(parent,value,acc){
	var panel = {
		...
		init:function(){
			...
			this.FForwardRule=createCheckBox("按倍数增加奖励","","");
			this.FForwardRead=createInput("转发阅读量:","",true);
			this.FForwardScore=createInput("转发阅读佣金:","",true);
			this.FMaxScore=createInput("最高奖励佣金:","",false);
			this.FExcType=createTwoCheckBox("排除员工转发","排除经纪人转发","","");
//			this.FExcType=createTwoCheckBox("员工转发不计算佣金","经纪人转发不计算佣金","","");
			...
			var this_FMaxScore = this.FMaxScore.p_inputs;
			var this_panel = this.p_panel;
			if(value["fid"]==undefined){
//				$(".input").numberbox('disable');
				this_FMaxScore.numberbox('disable');
			}
			$("#accordion").css("border-width","1px");
			
			
			this.FForwardRule.p_input.click(function(){
				if($(this).is(":checked")){
					this_FMaxScore.numberbox("enable");
				}else{
					this_FMaxScore.numberbox("disable");
				}
			});
...
function createCheckBox(title,value,explain){
	//var onclickStr="";
	//if(title=="按倍数增加奖励"){
	//	onclickStr = " onchange='fmusthave1(this)' ";
//		onclickStr = " onchange='multipleAdd(this)' ";
	//}
	var item = {
			p_fgroup:$('<div class="checkbox-parent" style="text-align: center;"></div>'),
			p_label:$('<label style="margin-top: 3px;display: inline-block;"></label>'),
			p_input:$('<input type="checkbox" style="margin-right:5px;vertical-align: middle;margin-top: 0px;"/>'+explain),
			init:function(){
				this.p_fgroup.append(this.p_input);
...
function createTwoCheckBox(title1,title2,value,explain){
	var item = {
			...
			p_label1:$('<label style="margin-top: 3px;display: inline-block;"></label>'),
			p_label2:$('<label style="margin-top: 3px;display: inline-block;"></label>'),
			p_input1:$('<input type="checkbox" style="margin-right:5px;vertical-align: middle;margin-top: 0px;"/>'+explain),
			p_input2:$('<input type="checkbox" style="margin-right:5px;vertical-align: middle;margin-top: 0px;"/>'+explain),
			init:function(){
...
function createTwoRadio(title1,title2,value,explain){
	var inputName = "FCalcType"+Math.random();
	var item = {
			...
			p_input1:$('<input type="radio" checked="checked" name="'+inputName+'" style="margin-right:5px;vertical-align: middle;margin-top: 0px;margin-bottom: 3px;"/>'+explain),
			p_input2:$('<input type="radio" name="'+inputName+'" style="margin-right:5px;vertical-align: middle;margin-top: 0px;margin-bottom: 3px;"/>'+explain),
			init:function(){
				this.p_span1.append(this.p_input1);
				this.p_span1.append(this.p_label1.text(title1));
				this.p_fgroup.append(this.p_span1);
