//新版easyUI活动管理	宣传设置	活动名称
var count = 0;
function jaccordionactivityNameList(parent,faccepted,width){   
	if(width==undefined){
		width = 400;//默认宽度
	}
	//border-width:0px;
	var accordion = {
		...
		p_add:$('<span style="margin-top:5px;">&nbsp;添加活动项目&nbsp;</span>'),
		p_data:new Array,
		p_index:0,
		//初始化
		init:function(){
			...
			this.p_add.linkbutton({
			    ...
			    iconCls	: 'icon-add',
			    onClick	: function(){
			    	this_accordion.p_data.push(addPanel1(this_group,"",this_accordion));
			    }
			});
		...
		setValue:function(g_data){
			...
			if(g_data==undefined||g_data==""||g_data.length==0){
				...
				for(var i = 0; i < g_data.length; i++){
					var panel = g_data[i];
					this.p_data.push(addPanel1(this.p_group,panel,this));
				}
				if(faccepted==1){
					releaseState();
				}
...
//创建一个手风琴项
function addPanel1(parent,value,acc){
	...
	var panel = {
		...
		init:function(){
			...
			this.FSourceID=createJeasyCombobox1("活动名称:","",false,"");
			//标签，标签描述，序号
...
//创建一个弹出选择
function createJeasyCombobox1(title,value,parent){
	var item = {
		...
		init:function(){
			...
			this.p_input.jeasycombo({
				...
				width : 350,
				//type : "list",
				//data : textMessages,//图文消息
				data : activityNameList,
				onChange : function(newValue,oldValue){
