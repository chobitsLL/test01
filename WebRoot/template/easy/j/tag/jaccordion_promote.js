//新版easyUI活动管理	宣传设置	图文消息
var count = 0;
function jaccordiontextMessages(parent,faccepted,width){   
	if(width==undefined){
		width = 400;//默认宽度
	}
	//border-width:0px;
	var accordion = {
		...
		p_add:$('<span style="margin-top:5px;">&nbsp;添加图文消息&nbsp;</span>'),
		p_data:new Array,
		p_index:0,
		//初始化
		init:function(){
...
function addPanel(parent,value,acc){
	...
	var panel = {
		...
		init:function(){
			...
			this.FSourceID=createJeasyCombobox("图文消息:","",false,"");
			//标签，标签描述，序号
...
//创建一个弹出选择
function createJeasyCombobox(title,value,parent){
	var item = {
...
function releaseState(){
	//$(".p_disablet").jeasycombo.disabled =true;
	//$(".p_disablet").jbootcombo("disabled",true);
	$(".p_disablet").jeasycombo("disabled","disabled");
}