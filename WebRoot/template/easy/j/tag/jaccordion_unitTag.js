...
function addPanel(parent,value,acc){
	var panel = {
		...
		//fcaptionid:$('<input type="hidden" />'),
		fid:$('<input type="hidden" />'),
		//取值
		getValue:function(){
			var v={};
			//v.fcaptionid=this.fcaptionid.val();
			v.fid=this.fid.val();
			v.fname=this.fcaptionname.getValue();
		...
		setValue:function(value){
			...
			this.isdelete = false;
			this.isupdate = true;
			this.fid.val(value["fid"]);
			this.fcaptionname.setValue(value["fcaptionname"]);
		...
		init:function(){
			...
			//标签，标签描述，序号
			//将输入项加入到panel中
			this.p_big.append(this.fcaptionname.p_fgroup);
			...
			var fcaptionname = this.fcaptionname.p_input;
			fcaptionname.textbox({
