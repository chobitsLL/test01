...
function jaccordion(parent,width){
	...
	var accordion = {
		p_group:$('<div class="panel-group" id="accordion" style="margin:0 auto;"></div>'),
...
function addPanel(parent,value,acc){
	...
	var panel = {
		...
		getValue:function(){
			...
			v.fisthanks =this.FIsThanks.getValue();
			if(typeid<5||typeid==7){
				v.fprobability =this.FProbability.getValue();
			}
			if(typeid==7){
				v.fstopwinning =this.FStopWinning.getValue();
			}
		...
		setValue:function(value){
			...
			this.FIsThanks.setValue(value["fisthanks"]);
			if(typeid<5||typeid==7){
				this.FProbability.setValue(value["fprobability"]);
			}
			if(typeid==7){
				this.FStopWinning.setValue(value["fstopwinning"]);
			}
		...
		deletePanel:function(){
			if(releaseedstate==1){
				$.messager.alert("提示","活动已发布,不能删除奖项！");
				return false;
			}else{
		...
		init:function(){
			...
			if(typeid==7){
				this.FIcon=createImg("奖项图标:","<font color='red' style='position: absolute;left: 100%;display: inline-block;width: 80%; text-align:left;'>*奖项图标尺寸如果大于50*50，会导致页面混乱！</font>",false);
			}else{
				this.FIcon=createImg("奖项图标:","",false);
			}
			if(typeid==7){
				this.FStopWinning=createcheckbox("是否只中一次奖:",0,"*如果选中则同一个粉丝只能中一次此奖项！",0);
				this.FIsThanks=createcheckbox("是否为谢谢参与:",0,"*该奖项是否为谢谢参与，不设置默认为非谢谢参与！",0);
			}else{
				this.FIsThanks=createcheckbox("是否为谢谢参与:",0,"*该奖项是否为谢谢参与，不设置默认为非谢谢参与！",1);
			}
			...
			if(typeid==7){
				this.p_big.append(this.FIsThanks.p_fgroup);
				this.p_big.append(this.FStopWinning.p_fgroup);
			}
...
function createSelect(title,value,parent){
	...
	if(typeid==7){
		options = [{fid:0,fname:"普通奖项",selected:true},{fid:3,fname:"积分"}];
	}
	var item = {
		p_fgroup:$('<div class="form-group" style="padding:1px;"></div>'),
		p_input:$('<input type="text" class="col-sm-9 form-control" style="width: 300px;" '+disabled+'/>'),
		init:function(){
			this.p_fgroup.append(this.p_input);
			var this_item = this;
...
function createUneditable(title){
	var item = {
		...
		p_input:$('<input type="text" class="" style="width: 250px;" disabled="true"/>'),
		init:function(){
			this.p_fgroup.append(this.p_input);
			this.p_input.textbox({
				label : title,
